...
class MissionGoal(models.Model):
    ...
    def clean(self):
        # 진행 중인 목표만 마감일 검증
        # 완료/포기된 목표는 이미 과거 날짜일 수 있으니 제외
        if self.status == self.Status.IN_PROGRESS:
            if self.deadline and self.deadline < timezone.now().date():
                raise ValidationError({"deadline": "마감일은 오늘 이후여야 합니다."})

    # ── 계산 프로퍼티 ──
    @property
    def current_save_amount(self):
        """
    ...
    def progress_rate(self):
        ...
        return round(
            (self.current_save_amount / self.target_price) * 100, 1
        )  # 소수점 첫째 자리까지만 반환
    ...
    def complete(self):
        """목표 완료 처리."""
        if self.status != self.Status.IN_PROGRESS:
            raise ValidationError("진행 중인 목표만 완료할 수 있 수 있습니다.")
    ...
    def __str__(self):
        return (
            f"{self.child.username} - {self.title} "
            f"({self.get_status_display()}, {self.progress_rate}%)"
        )
